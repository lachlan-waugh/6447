...
static char *get_store_dir(void)
{
    ...
    min_len += required;                // min_len is signed, required is unsigned
    char *def = malloc(min_len);
    if (def == NULL)
        return NULL;

    // Heap overflow, required should be min_len
...
static char *open_password_store(void)
{
    ...
    if (!((stat(pstore, &sb) == 0) && S_ISDIR(sb.st_mode))) {
        if (mkdir(pstore, S_IRWXU)) {
            ...
            // Doesn't exit on fail, returns failed directory
        }
    }
...
static char *get_passfile(const char *dir, const char *key)
{
    ...
    // Could overflow required (dir/key are user input)
    ...
    // Could overflow path with overflowed required
...
static int get_console_input(char *buf)
{
    ...
    char *buffer = malloc(4);       // doesn't check if allocated
    buf = NULL:                     // : not ;
    size_t cur_len = 0;

    /* read into buffer */
    while (fgets(buffer, sizeof(buffer), stdin) != 0) // sizeof(buffer) is sizeof(char *)
    {
    ...
    newline = '\0';                             // not derefencing newline, so sets to NULL
    free(buffer);
    return last;            // last is size_t (unsigned), returning as an int
}
...
static int init_crypto(void)
{
    ...
    if (key == NULL) {
      ...
      else {                // dangling else, brackets ended badly
        if (strlen(key) <= 0 || gpgme_get_key(cc.ctx, key, &cc.keylist[0], 0)) {     // order of operations
            return 1;
        }
...
static int insert_entry(const char *keyfile)
{
    ...
    // race condition (time of check, time of use), keyfile deleted after checking it exists
    ...
    char *input_buffer;
    size_t input_len = get_console_input(input_buffer); // input_len unsigned, console_input is int (signed)
    if (input_len < 0) {
...
static int get_entry(const char *keyfile)
{
    ...
    if (input_len < 0) { // doesn''t check if input_len == 0
    ...
    if(strcmp(input_buffer, pass)){ // could use timing attack to brute force for password
        free(input_buffer);         // use constant time string comparison
...
int main(int argc, const char **argv)
{
    ...
    //  get - return an entry
    //  del - delete an entry
    for(int i=1; i<=argc; i++){     // ret values don't exit, go to next loop. Off by one for <= argc
        if (strcmp("new", argv[1]) == 0) {
            ...
            ret = ERR_NO_CMD;  // doesn't check the value of ret
        }
    }
